...
class M_Cd extends CI_Model{
    ...
    public function SiSeDebeMostarProducto($idProducto) {
        ...
        if ($cont > 0) {//Existe el idcamiseta
            ...
            if ($datos['mostrarcat'] == 1 && $datos['mostrar'] == 1 && $datos['stock'] > 0)//Se debe mostrar la camiseta
                return true;
            else {
                return false;
            }
     ...
     * @param Int $idCategoria ID de la categoría
     * @param Int $idProducto ID de la camiseta
     * @return Array
     */
    ...
    public function getSeleccionadas($limit, $start) {
        ...
        $query = $this->db->query("SELECT pro.nombre_pro, pro.idProducto, pro.descripcion, pro.imagen, pro.precio, pro.descuento "
                . "FROM producto pro "
                . "INNER JOIN categoria cat on pro.idCategoria = cat.idCategoria "
                . "WHERE cat.mostrar=1 "
                . "AND pro.seleccionada = 1 "
                . "AND pro.mostrar = 1 "
                . "AND curdate() >= fecha_inicio "
                . "AND curdate() <= fecha_fin "
                . "AND stock > 0 "
